...
const Home = () => {
	...
	const [semaforoActual, semSemaforoActual] = useState('');
	...
	const creaSemaforos = ()=>{
		semSemaforoActual(colores.color.map((elemento, index)=>{
				console.log("Creando ", elemento, index); 
				return <button id={index} name={elemento} className={colores.activo[index] ? colores.normal[index] : colores.normal[index] } onClick={cambiaManual}></button>;
				
		}));
		console.log("Creado ", semaforoActual); 
...
//	function cambioAutomatico(){
	...
	return (
		<>
			...
			{console.log("semaforo:", semaforoActual)}
		</>
	);
};
